...
#
# Adapted by Just van den Broecke www.justobjects.nl mainly for use within CGI/MapScript
# - added hooks to call from other modules like CGI or programs
# - allow web-streaming of result image
# - Python syntax convention changes as per PEP8: http://www.python.org/dev/peps/pep-0008/
# - remove old-style class defs (with classname:())
# - made options function and local vars
# - no global vars anymore (concurrency/sync problem?)
# - extra option to pass in Point array created externally (e.g. from MapScript query)
...
class Projection:
    ...
    def AutoSetScale(self, bounding_box_ll, padding):
        if self.options.scale:
            ...
            pixels_per_degree = 111319.5 / self.options.scale   # px/deg = m/deg * px/m
        else:
            ...
            if self.options.height:
                pixels_per_degree = pixels_per_lat = float(
                        self.options.height - padding) / bounding_box_xy.SizeY() * SCALE_FACTOR
            if self.options.width:
                pixels_per_degree = float(self.options.width - padding) / bounding_box_xy.SizeX() * SCALE_FACTOR
                if self.options.height:
    ...
    def Project(self, (lat, lon)):
        ...
        try:
            y = -int(self.pixels_per_radian * log(
             tan((0.78539816339744828 + 0.0087266462599716477 * lat))))  # -int(log(tan(pi/4 + pi/180 * lat / 2)))
        except ValueError:  # sometimes a strange point
            y = 0
...
# Added by Just to test simpler (less CPU-intense) algorithms
class AddingMatrix(Matrix):
    def Add(self, coord, val):
        if coord in self.data:
            self.data[coord] += val
        else:
...
class ColorMap:
    ...
    def FromColorStopRgbaString(self, csrgba_str=None):
        from gradient import Gradient
        printtime('FromColorStopRgbaString:' + csrgba_str)
        self.FromImage(Gradient().create(csrgba_str).as_img())
...
class ImageMaker:
    ...
    def CreatePNG(self, matrix, requested_width=None, requested_height=None, bounding_box=None):
        printtime('CreatePNG: creating image')
        ...
        if self.background_image:
            self.image = Image.composite(self.image, self.background_image,
                                         self.image.split()[3])  # Is this really the best way?
...
def ChooseOSMZoom(options, bbox_ll, padding):
...
def GetOSMBackground(options, bbox_ll, padding):
    print 'GetOSMBackground..'
    zoom = ChooseOSMZoom(options, bbox_ll, padding)
    proj = MercatorProjection()
...
def printtime(str):
    if verbose:
        # Just : use stderr in order to print to Apache logfile in CGI mode
        sys.stderr.write('%10.3f sec\t // %s' % (time.time() - t0, str) + '\n')
...
def Finalize(options, matrix):
    printtime('Reducer: combining coincident points')
    dr = DiminishingReducer(options.decay)
    matrix = matrix.Reduce(dr.Reduce)
    return matrix

def ProcessShapes(options, shapes, projection, hook=None):
    printtime('ProcessShapes: processing data')
...
def setup_options():
    ...
    optparser.add_option('-p', '--points', metavar='FILE',
                         help='File containing one space-separated coordinate pair per line')
    ...
    optparser.add_option('-P', '--projection', metavar='NAME', type='choice', choices=projections.keys(),
                         default='mercator', help='choices: ' + ', '.join(projections.keys()) + '; default: %default')
    optparser.add_option('-e', '--extent', metavar='RANGE',
                         help='Clip results to RANGE, which is specified as lat1,lon1,lat2,lon2; (for square mercator: -85.0511,-180,85.0511,180)')
    optparser.add_option('-R', '--margin', metavar='INT', type='int', default=0,
                         help='Try to keep data at least this many pixels away from image border.')
    optparser.add_option('-r', '--radius', metavar='INT', type='int', default=15,
                         help='pixel radius of point blobs; default: %default')
    optparser.add_option('-d', '--decay', metavar='FLOAT', type='float', default=0.95,
                         help='float in [0,1]; Larger values give more weight to data magnitude.  Smaller values are more democratic.  default: %default')
    ...
    optparser.add_option('-f', '--frames', type='int', default=30,
                         help='number of frames for animation; default: %default')
    optparser.add_option('-F', '--ffmpegopts', metavar='STR',
                         help='extra options to pass to ffmpeg when making an animation')
    optparser.add_option('-K', '--keepframes', action='store_true',
                         help='keep intermediate images after creating an animation')
    optparser.add_option('-b', '--background', metavar='COLOR',
    ...
    optparser.add_option('-B', '--background_brightness', type='float', metavar='NUM',
                         help='Multiply each pixel in background image by this.')
    optparser.add_option('-m', '--hsva_min', metavar='HEX', default='#000ffff00',
                         help='#hhhssvvaa hex for minimum data values; default: %default')
    optparser.add_option('-M', '--hsva_max', metavar='HEX', default='#02affffff',
                         help='#hhhssvvaa hex for maximum data values; default: %default')
    optparser.add_option('-G', '--gradient', metavar='FILE',
                         help='Take color gradient from this the first column of pixels in this image.  Overrides -m and -M.')
...
def go(options):
# Just van den Broecke: no global options, gives synch problems in Web/CGI env
#    globals()['options'] = options
    ...
    if (options.gpx or options.points) and not (
    (options.width or options.height or options.scale or options.background_image)
    ...
    if options.output or options.web:
        ...
        if options.gradient:
            ...
            printtime('colormap: start from image')
            colormap.FromImage(Image.open(options.gradient))
        # Just: added allow on-the-fly gradient creation from colorstop rgba tuples (s,r,g,b,a),...
        elif options.gradient_color_srgba:
            printtime('colormap: start gradient_color_srgba  option')
            colormap.FromColorStopRgbaString(options.gradient_color_srgba)
        else:
            colormap.FromHsvaRangeStrings(options.hsva_min, options.hsva_max)

        printtime('colormap: created')
    ...
    if options.load:
        ...
        if options.gpx:
            ...
            f.close()
        elif options.points_arr:
            # Just van den Broecke: array of Point already created externally
            shapes = options.points_arr_arr
    ...
    if options.background_image:
        ...
        background_image, bounding_box_ll, projection = GetOSMBackground(options, bounding_box_ll,
    ...
    if not projection:
        projection = projections[options.projection](options)
    ...
    if not options.extent and verbose:
    ...
    if process_data:
        if options.animate:
            ...
            maker = ImageSeriesMaker(colormap, options.background, background_image, imgfile_template,
                                     min(options.frames, len(shapes)), len(shapes), options.width, options.height,
                                     bounding_box_xy)
            ...
            matrix = ProcessShapes(options, shapes, projection)
            matrix = Finalize(options, matrix)

    if not options.animate:
        imageMaker = ImageMaker(colormap, options.background, background_image)
        imageMaker.CreatePNG(matrix, options.width, options.height, bounding_box_xy)
        printtime('Image created, streaming or saving it..')

        if options.output:
            imageMaker.SavePNG(options.output)
        elif options.web:
...
# from another module or (CGI) program
def main(args=None, points=None):
